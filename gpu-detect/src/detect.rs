...
impl GpuDetector {
    ...
    fn detect_amd() -> Option<GpuInfo> {
        ...
        if vram_gb > 0.0 || gpu_name.is_some() || architecture.is_some() {
            ...
            debug!(
                " AMD GPU detected: {} with {:.1}GB VRAM",
                final_gpu_name, final_vram
            );
            ...
            for line in info.lines() {
                    ...
                    && (line.contains("VGA") || line.contains("Display"))
                {
                    ...
                    return Some(GpuInfo {
                        gpu_type: format!(
                            "AMD GPU ({})",
                            line.split(':').last().unwrap_or("Unknown").trim()
                        ),
                        vram_gb: vram,
                        compute_capability: None,
                        driver_version: None,
    ...
    fn extract_memory_from_line(line: &str) -> Option<f64> {
        ...
        let memory_keywords = vec!["vram", "memory", "mem", "total", "size", "dedicated"];

        let has_memory_keyword = memory_keywords
            .iter()
            ...
            for part in parts {
                ...
                if !number_part.is_empty() {
                    if let Ok(value) = number_part.parse::<f64>() {
                        ...
                        if gb_value >= 1.0 && gb_value <= 128.0 {
                            debug!(
                                " Extracted {:.1}GB from colon-separated: '{}'",
                                gb_value,
                                line.trim()
                            );
                            return Some(gb_value);
    ...
    fn infer_amd_vram_from_name(gpu_name: &str) -> f64 {
        ...
        if name_lower.contains("6500 xt") {
            return 4.0;
        }
        if name_lower.contains("5700 xt") {
            return 8.0;
        }
        if name_lower.contains("5700") {
            return 8.0;
        }
    ...
    fn infer_amd_vram_from_lspci(lspci_line: &str) -> f64 {
        ...
        if line_lower.contains("7800 xt") {
            return 16.0;
        }
        if line_lower.contains("7900 xtx") {
            return 24.0;
        }
        if line_lower.contains("7900 xt") {
            return 20.0;
        }
        if line_lower.contains("7700 xt") {
            return 12.0;
        }
        if line_lower.contains("6900 xt") || line_lower.contains("6800") {
            return 16.0;
        }
        if line_lower.contains("6700 xt") {
            return 12.0;
        }
        if line_lower.contains("6600") {
            return 8.0;
        }
                ...
                if output.status.success() {
                    ...
                    if info.to_lowercase().contains("intel") {
                        ...
                        for line in info.lines() {
                            ...
                            {
                                let device_name =
                                    line.split(':').nth(1).unwrap_or("Intel GPU").trim();
