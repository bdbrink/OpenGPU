...
impl MLReadinessChecker {
    ...
    fn infer_model_specs(model_id: &str) -> ModelInfo {
        ...
        ModelInfo {
            ...
            description: format!(
                "Popular model from {}",
                model_id.split('/').next().unwrap_or("community")
            ),
        }
    }
    ...
    pub fn evaluate_readiness(specs: &SystemSpecs) -> ReadinessLevel {
        ...
        if specs.total_ram_gb >= 64.0 {
            score += 30;
        } else if specs.total_ram_gb >= 32.0 {
            score += 25;
        ...
        if specs.gpu_info.is_ml_ready {
            if specs.gpu_info.vram_gb >= 24.0 {
                score += 40;
            } else if specs.gpu_info.vram_gb >= 16.0 {
                score += 35;
            } else if specs.gpu_info.vram_gb >= 12.0 {
                score += 30;
            } else if specs.gpu_info.vram_gb >= 8.0 {
                score += 25;
            } else if specs.gpu_info.vram_gb >= 6.0 {
                score += 20;
            } else if specs.gpu_info.vram_gb >= 4.0 {
                score += 15;
            } else {
                score += 10;
            }
        ...
        if specs.disk_space_gb >= 500.0 {
            score += 10;
        } else if specs.disk_space_gb >= 100.0 {
            score += 8;
        } else if specs.disk_space_gb >= 50.0 {
            score += 5;
        } else {
            score += 2;
        }
    ...
    pub async fn get_model_recommendations(specs: &SystemSpecs) -> Vec<ModelRecommendation> {
        ...
        let large_models: Vec<ModelInfo> = models
            .iter()
            .filter(|m| {
                m.min_vram_gb >= 24.0
                    && specs.gpu_info.vram_gb >= m.min_vram_gb
                    && specs.total_ram_gb >= m.min_ram_gb
            })
            .cloned()
            .take(3)
            .collect();

        let medium_models: Vec<ModelInfo> = models
            .iter()
            .filter(|m| {
                m.min_vram_gb >= 6.0
                    && m.min_vram_gb < 24.0
        ...
        let small_models: Vec<ModelInfo> = models
            .iter()
            .filter(|m| {
                m.min_vram_gb < 6.0
                    && (specs.gpu_info.vram_gb >= m.min_vram_gb
    ...
    fn print_improvement_suggestions(specs: &SystemSpecs) {
        ...
        if !specs.gpu_info.is_ml_ready {
            println!(
                "🔧 A dedicated GPU with 8GB+ VRAM would significantly improve ML capabilities"
            );
